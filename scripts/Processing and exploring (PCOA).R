...
##Extra filtering
###########################################################
##May be too intensive for PC, or more useful for pop stats etc.
###########################################################

##  Filtering out over-split loci (likely already done by DArT but need to use UNE computation)
...
##  size, so choose a threshold based on your gl.report.heterozygosity (don't undertsand why I
##  would filter out samples with high hterozygosity).
# hist(gl.Ho(mygl))
# gl.report.heterozygosity(mygl1)
## Recalc Observed heterozygosity histogram
# hist(gl.Ho(mygl1))
