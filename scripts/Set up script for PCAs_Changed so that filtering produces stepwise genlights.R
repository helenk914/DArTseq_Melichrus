...
if(runfilt){
  ...
  mygl2 <- gl.filter.RepAvg(mygl1,threshold = .98)
  ## Remove monomorphic loci
  mygl3 <- gl.filter.monomorphs(mygl2)
  ## Filter out large amounts of missing data by locus
  mygl4 <- gl.filter.callrate(mygl3,method = "loc",threshold = 0.98)
  ##  Thin to one snp per tag. Usually best to choose the one with the highest minor allele frequency.
  mygl5 <- gl.filter.secondaries(mygl4,method = "best")
  ## Filter out individuals with too much missing data
  mygl6 <- gl.filter.callrate(mygl5,method = "ind",threshold = 0.9,recalc = TRUE, mono.rm = TRUE)
  ...
  mygl7 <- gl.filter.maf(mygl6,threshold = 0.05)
