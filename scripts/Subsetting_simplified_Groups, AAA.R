...
for (subsets in c("AAA","BBA","BAA", "AAB")){
  Subsetgl <- mygl[match(metadata$id[which(metadata$Tertiary_PCA_group==subsets)],mygl$ind.names),]
...
## Ordination
##################################################################
load(file = file.path("temp",paste0("SubFiltglAAB.rda")))
nLoc(Subsetgl1);nInd(Subsetgl1)
#Subsetgl1
